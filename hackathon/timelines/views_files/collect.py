...
def keywords(key_body):
    ...
    company = ('Company' or 'Position_Vocation' or 'Detective_Method_Other' or 'Product_Other' or 'Corporation_Other')
    project = ('Product_Other' or 'Magagine' or 'Broadcast_Program' or 'Occation_Other' or 'Flora' or 'Person')
...
def serch(body, prolist):
    ...
    list_1 = allpro.keys()      #事業名の配列
    list_2 = allpro.values()    #配列の配列
    list_3 = []
    l = len(list_2)
    for k in range(l):
    ...
    for m in range( len(list_2(list_3.index(max))) ):
    ...
    list_4 = allpro.keys()      #企業名の配列
    list_5 = allpro.values()    #配列の配列
    list_6 = []
    l = len(list_5)
    for k in range(l):
    ...
    tree = []
    for n in range( len(list_2(list_3.index(max))) ) :
        data = {
            'company_id':   body[list_2[n]]['company_id'],          #body_temp["company_id"]
            'release_id':   body[list_2[n]]['release_id'],      #body_temp["release_id"]
            ...
            'project_name': list_1[list_3.index(max)],
            'company_name': list_4[list_6.index(max)]
        }
        tree.apend(data)
    return tree
